...
| `linktrend-review-ready-publisher.yml` | Trusted built-in-token `workflow_dispatch` publisher/withdrawer for `Linktrend Review Ready` (`action=publish|withdraw`; flag only on publish/withdraw; default-branch scripts; tip is data only) |
