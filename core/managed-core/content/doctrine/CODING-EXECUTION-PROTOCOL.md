...
| Hosted-capacity scheduler | Deterministic adaptive admission runtime (`core/execution/scheduler.py`) using live authenticated Cursor capacity, dependency/path readiness, a configured spend ceiling, and explicit safety limits. No fixed hosted maximum is encoded. Capacity evidence must match the exact account, API-key name, team, and Program Run identity. Unknown or stale evidence blocks admission; free slots plus waiting work emit `UTILIZATION_GAP` and recompute; paid/Fast fallback is forbidden. |
