...
Use at the **start of a NEW agent session** to bootstrap onto a short-lived `issue/<id>-<slug>` branch from latest `development`.
...
House rule: one short-lived `issue/*` per governed work — not forever `dev/*`. Cloud uses `cursor/*`; `dev/*` rare ad-hoc only.
