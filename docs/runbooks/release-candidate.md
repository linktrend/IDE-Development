...
4. Package identity consistent: root `VERSION`, `core/managed-core/VERSION`, and `MANIFEST.json` `packageVersion` (target **2.1.4** / `v2.1.4`).
...
Tag and GitHub Release creation for managed-core **v2.1.4** is owned by the system-repository workflow `.github/workflows/linktrend-managed-core-release-publisher.yml` (not consumer-synced).
