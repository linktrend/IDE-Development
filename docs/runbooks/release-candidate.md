...
4. Package identity consistent: root `VERSION`, `core/managed-core/VERSION`, and `MANIFEST.json` `packageVersion` (target **2.3.3** / `v2.3.3`).
...
Tag and GitHub Release creation for managed-core **v2.3.3** is owned by Terra/operator release procedure and the system-repository workflow `.github/workflows/linktrend-managed-core-release-publisher.yml` (not consumer-synced). W3-P1 packet execution does not perform publication.
