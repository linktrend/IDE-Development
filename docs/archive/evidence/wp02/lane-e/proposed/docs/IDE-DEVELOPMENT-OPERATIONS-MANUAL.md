...
| Real consumer rollout | **Deferred.** Work Packet 1 (Issue #67) proved a release candidate on disposable targets. Work Packet 2 (Issue #68) does **not** mutate consumers. Installing real product repos needs separate Principal approval after Work Packet 3 publication decisions (`docs/GITOPS-CONSUMER-ROLLOUT.md`). |
| Git tag / GitHub Release | **Not created in WP1 or WP2.** An RC archive may be built for proof only. |
| Live GitHub settings apply | **WP1 = plan/verify only.** WP2 may apply IDE Development external state only via authorized GitHub App path with restorable before-state; never consumers. |
...
Issue #68 — currently in progress. After WP1’s release-candidate proof, WP2 builds one clean issue-branch lineage (development + WP01 + cleanup + any unique still-required content), hardens stale-cleanup controls (plan only), and verifies IDE Development live external readiness. It stops at a pushed checkpoint. It does **not** merge into `development`, promote, publish tag/Release, or roll out consumers.

**What is Work Packet 3?**
Integration into `development` / promotion / publication decisions under your separate approval. Consumer installs remain separately Principal-gated.
