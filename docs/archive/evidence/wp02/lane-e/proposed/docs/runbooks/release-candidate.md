...
Work Packet 1 **prepares and proves** the RC. Work Packet 2 (Issue #68) incorporates that RC lineage into a canonical issue-branch checkpoint and live-readiness posture. Work Packet 3 handles integration into `development` / publication decisions. Consumer rollout remains separately Principal-gated.
...
| Merge into `development` / promote | **Work Packet 3** (not WP2) |
| Real consumer install/update | **Deferred** — separately Principal-gated; see consumer rollout doc |
| Live GitHub App / secrets / Bugbot / ruleset apply | **Forbidden in WP1**; WP2 may apply IDE Development only under packet constraints |
| Claude packaging or docs claiming Claude support | **Forbidden** |

---

## Work Packet 2 / Work Packet 3 boundaries

**Work Packet 2** (Issue #68, in progress) builds the canonical lineage, hardens stale-cleanup (plan only), and verifies/applies IDE Development live external readiness. It ends at a pushed issue-branch checkpoint. It does **not** merge into `development`, promote, publish tag/Release, or mutate consumers.

**Work Packet 3** is the **integration and publication** stage: governed merge into `development` / promotion and final publication/rollout **decisions** under **separate** Carlos approval. WP1 stops at a proven RC + evidence on the issue branch.
