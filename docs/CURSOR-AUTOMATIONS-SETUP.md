...
Otherwise skip creating Automations; Lisa owns the schedule (Ship 05:00, Pull 07:00, Ship 16, Pull 18 Asia/Taipei).
...
- After each run, update Lisa status file when the agent can write local files:
  ...
  - One line only, e.g. `Ship 05:00: Clear` or `Pull 18: Issues`

## Automation 1 — Ship 05:00 — backup only
...
```text
Ship 05:00 (Asia/Taipei). You are the Implementer under IDE Development autonomous Git ops.
...
to exactly: Ship 05:00: Clear
or Ship 05:00: Issues
(no lists, no links).

Reply with that same one line only.
```

## Automation 2 — Pull 07:00 — backup only
...
```text
Pull 07:00 (Asia/Taipei). Sync to latest development.
Primary clock is Lisa Option A; you are a backup Automation run.
...
Same as Ship 05:00; replace labels with `Ship 16`.

## Automation 4 — Pull 18 — backup only

Same as Pull 07:00; replace labels with `Pull 18`.
