...
| Consumer managed-file immutability | Treat every installed IDE Development 2.5.1 managed file as read-only and byte-identical to the official 2.5.1 package. Do not permit consumer-local patches; preserve legitimate repository work, then restore drift only through the official 2.5.1 installer/manifest. | Required for the future release contract; all consumers remain on 2.5.1. |
...
### 15.2.1 Consumer 2.5.1 immutability and repair contract

Until a separately defined and approved 2.5.2 release exists:

1. Every consumer's installed IDE Development managed file must remain read-only and identical to the official 2.5.1 installer/manifest output. A changed managed byte is drift, not a consumer customization.
2. Consumer repositories must not carry local patches to managed IDE Development files. Legitimate repository-owned product or configuration work must be preserved before any drift repair is attempted.
3. Drift is restored only by the official 2.5.1 installer/manifest transaction. Agents must not hand-edit, copy from another consumer, or selectively overwrite managed files to repair a mismatch.
4. Proposed managed-file changes are central 2.5.2 revision candidates. They may be queued, implemented, audited, and protected-integrated centrally, but they are not 2.5.1 changes and do not authorize a consumer update.
5. The Issue #428 Review Ready publisher import repair and its tests are queued under this rule. The accepted checkpoint and closed PR #429 are evidence for candidate provenance only; they are not 2.5.1 release or rollout evidence.
