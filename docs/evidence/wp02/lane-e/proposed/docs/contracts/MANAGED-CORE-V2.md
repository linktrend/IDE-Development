...
<repo>/
  .ide-development/                 # committed physical managed package (required)
    VERSION                         # package semver string, e.g. 2.0.0
...
| `packageVersion` | Semver package version (`2.0.0`) |
...
| Consumer rollout | Other LiNKtrend repos | **Deferred** in WP1 and WP2. Inventory + Principal approval gate in `docs/GITOPS-CONSUMER-ROLLOUT.md`. Work Packet 2 is lineage + live readiness (no consumer mutation). Work Packet 3 handles integration/publication decisions. |
