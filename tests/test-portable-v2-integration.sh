...
assert_packaging() {
  ...
  [[ "${root_ver#v}" == "2.2.0" ]] || fail "root VERSION must be v2.2.0 identity (got '$root_ver')"
  [[ "${pkg_ver#v}" == "2.2.0" ]] || fail "managed VERSION must be 2.2.0 identity (got '$pkg_ver')"
  ...
  [[ "$pkg_json_ver" == "2.2.0" ]] || fail "MANIFEST packageVersion must be 2.2.0 (got '$pkg_json_ver')"
  [[ "${pkg_ver#v}" == "$pkg_json_ver" ]] || fail "managed VERSION ($pkg_ver) != packageVersion ($pkg_json_ver)"
  pass "VERSION / packageVersion aligned at 2.2.0"
...
assert_docs() {
  ...
  [[ "$ver" == "v2.2.0" ]] || fail "VERSION must be v2.2.0 (got '$ver')"
  pass "VERSION is v2.2.0"
...
PY
  ...
  pass "v2.2.0 identified without tag/release claim"
