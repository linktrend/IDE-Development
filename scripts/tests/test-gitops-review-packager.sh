...
grep -q 'workflow_dispatch:' "$PKG" || fail "packager missing reconciled-tree Fast dispatch"
grep -q 'options: \[reconciled\]' "$PKG" || fail "Fast dispatch must be restricted to reconciled mode"
grep -q 'ref: development' "$PKG" || fail "Fast dispatch must check out trusted development only"
grep -q 'persist-credentials: false' "$PKG" || fail "Fast dispatch must not persist credentials"
grep -q 'verify_reconciled_fast_dispatch.py' "$PKG" || fail "Fast dispatch must bind exact installed identity"
! grep -q 'actions/cache' "$PKG" || fail "Fast dispatch must not expose cache poisoning surface"
...
grep -q 'workflow_dispatch:' "$INT" || fail "Full canary dispatch trigger missing"
grep -q 'github.ref_name' "$INT" || fail "Full canary dispatch must bind candidate ref"
