...
def _self_test() -> int:
    ...
    ok = expect_ok(source_sha=good_sha, version="2.5.0", tag="v2.5.0", dry_run=False)
    if ok:
        assert ok.tag == "v2.5.0"
    ...
    expect_err("tag_version_mismatch", source_sha=good_sha, version="2.5.0", tag="v9.9.9")
